...
def draw_sphere_quick(xCen, yCen, zCen, radius): # Removes the error checking from sphere calculations, as it is unnnessisary fro multi plots
    theta = linspace(0, 2 * pi, 500)
    phi = linspace(0, pi, 500)
    # theta and phi are 500 angles in radians between 0 and 2pi and 0 and pi respectivly, these are used to create the points on the sphere to plot.
    red = 110
...
def single_Sphere_plot(radius):
    from mpl_toolkits.mplot3d import Axes3D
    import matplotlib.pyplot
    (x, y, z) = draw_sphere(0, 0, 0, radius)
...
def multiSpherePlot(size):
    ...
    for i in range(size):
        ...
        for j in range(size):
            for k in range(size):
                (xPlt, yPlt, zPlt) = draw_sphere_quick(xCen + (i * size), yCen + (j * size), zCen + (k * size), (size * 0.2))
...
def compoundPlot(cCx, cCy, cCz, scalingF, size,
                 ax):  # takes center of the sodium chloride compound and builds visualiseation around
    import matplotlib.pyplot
    from mpl_toolkits.mplot3d import Axes3D
    (xPlt, yPlt, zPlt) = draw_sphere_quick(cCx, cCy, cCz, size * 0.2)
    ax.plot_surface(xPlt, yPlt, zPlt, color='red')
    (xPlt, yPlt, zPlt) = draw_sphere_quick(cCx + scalingF, cCy, cCz, size * 0.1)
    ax.plot_surface(xPlt, yPlt, zPlt, color='blue')
    (xPlt, yPlt, zPlt) = draw_sphere_quick(cCx, cCy + scalingF, cCz, size * 0.1)
    ax.plot_surface(xPlt, yPlt, zPlt, color='blue')
    (xPlt, yPlt, zPlt) = draw_sphere_quick(cCx, cCy, cCz + scalingF, size * 0.1)
    ax.plot_surface(xPlt, yPlt, zPlt, color='blue')
    (xPlt, yPlt, zPlt) = draw_sphere_quick(cCx - scalingF, cCy, cCz, size * 0.1)
    ax.plot_surface(xPlt, yPlt, zPlt, color='blue')
    (xPlt, yPlt, zPlt) = draw_sphere_quick(cCx, cCy - scalingF, cCz, size * 0.1)
...
def sodiumPlot(size):
    ...
    for i in range(2, size, 4):
        ...
        for j in range(2, size, 4):
            for k in range(2, size, 4):
                compoundPlot(xCen + i, yCen + j, zCen + k, 2, 8, ax)
    #matplotlib.pyplot.draw()
    matplotlib.pyplot.show()
